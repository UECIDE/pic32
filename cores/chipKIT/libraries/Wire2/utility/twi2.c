...
static volatile uint8_t twi2_state;
static uint8_t twi2_slarw;

static volatile uint8_t TW_STATUS;

static void (*twi2_onSlaveTransmit)(void);
static void (*twi2_onSlaveReceive)(uint8_t*, int);

static uint8_t twi2_masterBuffer[TWI2_BUFFER_LENGTH];
static volatile uint8_t twi2_masterBufferIndex;
static uint8_t twi2_masterBufferLength;

static uint8_t twi2_txBuffer[TWI2_BUFFER_LENGTH];
static volatile uint8_t twi2_txBufferIndex;
static volatile uint8_t twi2_txBufferLength;

static uint8_t twi2_rxBuffer[TWI2_BUFFER_LENGTH];
static volatile uint8_t twi2_rxBufferIndex;
static volatile uint8_t twi2_rxBufferLength;

static volatile uint8_t twi2_error;

/* Forward declarations for internal utility functions.
*/
uint32_t twi2_computeBrg(uint32_t frqReq);
void __attribute__((interrupt(),nomips16)) I2C2Handler(void);
 ...
 * Output   none
 */
void twi2_init(p32_i2c * ptwiT, uint8_t irqBus, uint8_t irqSlv, uint8_t irqMst, uint8_t vec)
{
	p32_regset *	pregIpc;
	int				bnVec;
    ...
    setIntVector(_TWI2_VECTOR, I2C2Handler);
	...
	*/
	...
	pregIpc->set = ((_TWI2_IPL << 2) + _TWI2_SPL) << bnVec;
	...
	*/
	ptwi->ixBrg.reg = twi2_computeBrg(TWI2_FREQ);

}

/* 
 * Function twi2_slaveInit
 * Desc     sets slave address
 * Input    none
 * Output   none
 */
void twi2_setAddress(uint8_t address)
{
 ...
 */
uint8_t twi2_readFrom(uint8_t address, uint8_t* data, uint8_t length)
{
  unsigned int i;
  ...
  twi2_masterBufferIndex = 0;
  twi2_masterBufferLength = length;

   // send start condition
  TW_STATUS = TW_START;
  twi2_slarw = (address << 1) + 1;
  ...
  length = twi2_masterBufferIndex;

  // copy twi buffer to data
  for(i = 0; i < length; i++){
    data[i] = twi2_masterBuffer[i];
  }
 ...
 */
uint8_t twi2_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait)
{
  uint8_t i;
  ...
  for (i = 0; i < length; ++i)
  {
    twi2_masterBuffer[i] = data[i];
  }
  
  // send start condition
  TW_STATUS = TW_START;
  twi2_slarw = (address << 1);
  ptwi->ixCon.set = (1 << _I2CCON_SEN);
  ...
  else if (twi2_error == TW_MT_SLA_NACK)
  {
    return 2;	// error: address send, nack received
  }
  else if (twi2_error == TW_MT_DATA_NACK)
  {
 ...
 *          0 ok
 */
uint8_t twi2_transmit(uint8_t* data, uint8_t length)
{
  uint8_t i;
  ...
  twi2_txBufferLength = length;
  for(i = 0; i < length; ++i
 ){
    twi2_txBuffer[i] = data[i];
  }
...
/* 
 * Function twi2_attachSlaveRxEvent
 * Desc     sets function called before a slave read operation
 * Input    function: callback function to use
 * Output   none
 */
void twi2_attachSlaveRxEvent( void (*function)(uint8_t*, int) )
{
  twi2_onSlaveReceive = function;
}

/* 
 * Function twi2_attachSlaveTxEvent
 * Desc     sets function called before a slave write operation
 * Input    function: callback function to use
 * Output   none
 */
void twi2_attachSlaveTxEvent( void (*function)(void) )
{
  twi2_onSlaveTransmit = function;
}

/* ---------- Internal Utility Functions ---------- */
/* 
 * Function twi2_computeBrg
...
uint32_t twi2_computeBrg(uint32_t frqReq)
{
	...
	*/
	...
	return (CLK_PBUS / (2 * TWI2_FREQ) - tpgd) - 2;
}		

/* ---------- Interrupt Service Routine ---------- */
/* 
 * Function I2C2Handler
	...
	switch (TW_STATUS)
	{
		...
		case TW_MT_DATA:
			...
			else
			{
				...
				if (twi2_masterBufferIndex < twi2_masterBufferLength)
				{
					// More data to send load transmit buffer
					ptwi->ixTrn.reg = twi2_masterBuffer[twi2_masterBufferIndex];
					twi2_masterBufferIndex++;
				}
				else
				{
		...
		case TW_MR_DATA: // data received
			...
			twi2_masterBuffer[twi2_masterBufferIndex] = ptwi->ixRcv.reg;
			twi2_masterBufferIndex++;
		...
		case TW_SR_DATA:
			twi2_rxBuffer[0] = ptwi->ixRcv.reg;
			twi2_onSlaveReceive(twi2_rxBuffer, 1);
			// Release clock line
			ptwi->ixCon.set = (1 << _I2CCON_SCLREL);
		...
		//Slave Transmitter
		case TW_ST_SLA:
			twi2_txBufferIndex = 0;
			twi2_txBufferLength = 0;
			TW_STATUS = TW_ST_DATA;
			
			twi2_onSlaveTransmit();

			if (0 == twi2_txBufferLength)
			{
				twi2_txBufferLength = 1;
				twi2_txBuffer[0] = 0x00;
		...
		case TW_ST_DATA:
			...
			else
			{
				...
				if(twi2_txBufferIndex < twi2_txBufferLength)
				{
					ptwi->ixTrn.reg = twi2_txBuffer[twi2_txBufferIndex++];			
				}
				else
				{
					ptwi->ixTrn.reg = 0;
