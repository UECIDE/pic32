...
uint32_t TwoWire2::beginCount = 0;
...
void TwoWire2::begin(void)
{
    ...
    // Not only will this work around the multiple twi2_init bug, but
    ...
    if (beginCount == 1) { // First init
        ...
        twi2_init((p32_i2c *)_TWI2_BASE, _TWI2_BUS_IRQ, _TWI2_SLV_IRQ, _TWI2_MST_IRQ, _TWI2_VECTOR);
    }
}
...
void TwoWire2::begin(uint8_t address)
{
    ...
    if (beginCount == 1) { // First init
        twi2_setAddress(address);
        twi2_attachSlaveTxEvent(onRequestService);
        twi2_attachSlaveRxEvent(onReceiveService);
    }
}
...
//
// Until it is completed this is an option function enabled in the Wire2.h header file.
...
uint8_t TwoWire2::requestFrom(uint8_t address, uint8_t quantity)
{
  ...
  uint8_t read = twi2_readFrom(address, rxBuffer, quantity);
...
uint8_t TwoWire2::endTransmission(void)
{
  ...
  int8_t ret = twi2_writeTo(txAddress, txBuffer, txBufferLength, 1);
  // reset tx buffer iterator vars
  txBufferIndex = 0;
  txBufferLength = 0;
...
int TwoWire2::write(uint8_t data)
{
    if(transmitting) {
        ...
        // in slave send mode
        // reply to master
        twi2_transmit(&data, 1);
    }
    return 1;
}
void TwoWire2::send(uint8_t data) { write(data); }

// must be called in:
// slave tx event callback
// or after beginTransmission(address)
int TwoWire2::write(uint8_t* data, uint8_t quantity)
{
    if(transmitting){
        // in master transmitter mode
        ...
        // in slave send mode
        // reply to master
        twi2_transmit(data, quantity);
    }
    return 1;
}
void TwoWire2::send(uint8_t* data, uint8_t quantity) { write(data, quantity); }

// must be called in:
// slave tx event callback
// or after beginTransmission(address)
int TwoWire2::write(char* data)
{
    return write((uint8_t*)data, strlen(data));
}
void TwoWire2::send(char* data) { write(data); }

// must be called in:
// slave tx event callback
// or after beginTransmission(address)
int TwoWire2::write(int data)
{
    return write((uint8_t)data);
}
void TwoWire2::send(int data) { write(data); }
